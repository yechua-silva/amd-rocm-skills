#!/bin/bash
#===============================================================================
# Munin — Entrypoint con Detección Automática de Backend GPU
#===============================================================================
...
if [ -n "$BACKEND" ] && [ "$BACKEND" != "auto" ]; then
    case "$BACKEND" in
        cuda|rocm|cpu)
            echo "[Munin] Backend forzado: $BACKEND" >&2
            export BACKEND
            exec "$@"
            ;;
        *)
            echo "[Munin] ⚠️  BACKEND desconocido: '$BACKEND'. Procediendo con auto-detección." >&2
            ;;
    esac
fi
...
# ── Banner ────────────────────────────────────────────────────────────────────
echo "[Munin] ╔═══════════════════════════════════════════╗" >&2
echo "[Munin] ║   Munin — GPU Backend Detection          ║" >&2
echo "[Munin] ╚═══════════════════════════════════════════╝" >&2
echo "" >&2

# ── Nivel 1: Detectar NVIDIA CUDA ─────────────────────────────────────────────
echo "[Munin] 🔍 Detectando NVIDIA CUDA..." >&2
...
if command -v nvidia-smi &> /dev/null; then
    ...
    if [ "$NVIDIA_COUNT" -gt 0 ]; then
        ...
        echo "[Munin] ✅ NVIDIA CUDA detectado: $NVIDIA_COUNT GPU(s) — $NVIDIA_NAME" >&2
        echo "[Munin]    Driver: $NVIDIA_DRIVER" >&2
        export BACKEND=cuda
        export NVIDIA_VISIBLE_DEVICES=${NVIDIA_VISIBLE_DEVICES:-all}
        export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-0}
        echo "[Munin] ✅ Backend configurado: BACKEND=$BACKEND" >&2
        exec "$@"
    else
        echo "[Munin] ⚠️  nvidia-smi encontrado pero sin GPUs detectadas" >&2
    fi
else
    echo "[Munin]    nvidia-smi no disponible" >&2
fi

# ── Nivel 2: Detectar AMD ROCm vía rocm-smi ──────────────────────────────────
echo "[Munin] 🔍 Detectando AMD ROCm..." >&2
...
" 2>/dev/null || echo 0)
    ...
    if [ "$ROCM_CARD_COUNT" -gt 0 ]; then
        echo "[Munin] ✅ AMD ROCm detectado vía rocm-smi: $ROCM_CARD_COUNT GPU(s)" >&2
        ROCM_DETECTED=true
    else
        echo "[Munin] ⚠️  rocm-smi disponible pero sin GPUs reportadas" >&2
    fi
else
    echo "[Munin]    rocm-smi no disponible" >&2
fi

# ── Nivel 3: Detectar AMD ROCm vía dispositivos ──────────────────────────────
if ! $ROCM_DETECTED; then
    if [ -e /dev/kfd ]; then
        echo "[Munin] ✅ /dev/kfd presente — posible GPU AMD" >&2
        ROCM_DETECTED=true
    fi

    if ls /dev/dri/render* &> /dev/null 2>&1; then
        RENDER_COUNT=$(ls /dev/dri/render* 2>/dev/null | wc -l)
        echo "[Munin] ✅ /dev/dri/render* presente ($RENDER_COUNT nodos) — posible GPU AMD" >&2
        ROCM_DETECTED=true
    fi
    ...
    if command -v rocminfo &> /dev/null; then
        ...
        if [ -n "$ROCMINFO_GFX" ]; then
            echo "[Munin] ✅ rocminfo: arquitectura $ROCMINFO_GFX detectada" >&2
            ROCM_DETECTED=true
        fi
    fi
fi
...
echo "[Munin] ⚠️  No se detectó GPU — usando CPU fallback" >&2
export BACKEND=cpu
...
echo "[Munin]    OMP_NUM_THREADS=$OMP_NUM_THREADS" >&2
echo "[Munin] ✅ Backend configurado: BACKEND=$BACKEND" >&2
